#!/usr/bin/env python3
"""
Clean v2 plot: POS-level word-order alignment for sweep_apr16 (fixed budget).

Adapted from apr14/pos_syntax_plot_v2.py. Uses all 3 seeds (apr16 has
the fixed-budget design, so all seeds should be stable).
"""
import pandas as pd
import numpy as np
import matplotlib
...
OUT = Path("/n/home06/drooryck/codeswitching-llms/april_exp/cfg/scripts/apr16")

df = pd.read_csv(OUT / "syntax_divergent_detail.csv")
print(f"Loaded {len(df)} rows", flush=True)
print(f"Seeds: {sorted(df['seed'].unique())}", flush=True)

# ── Use all seeds (fixed-budget should be stable) ──────────────────
df_all = df.copy()
...
df_all["role_pair"] = df_all["en_role"] + "→" + df_all["nl_role"]

# ═══════════════════════════════════════════════════════════════════
# PLOT 1: Main overview — NL test, grouped by category
# ═══════════════════════════════════════════════════════════════════
nl = df_all[df_all["test_lang"] == "nl"]
...
n_seeds = len(df_all['seed'].unique())
ax.set_title(f"Which POS positions follow English word order?\n"
             f"(Dutch test sentences, {n_seeds} seeds, tf=0, fixed budget)",
...
fig2.suptitle(f"Word-Order Alignment by Structure Type (apr16, fixed budget)\n"
              f"(Dutch test sentences, {n_seeds} seeds, tf=0)",
...
fig3.suptitle(f"POS-Level Language Alignment in Dutch Predictions (apr16)\n"
              f"({n_seeds} seeds, tf=0 — 0 = Dutch order, 1 = English order)",
              fontsize=13, fontweight="bold")
fig3.tight_layout()
...
# ═══════════════════════════════════════════════════════════════════
# PLOT 4: Faceted heatmaps by structure
# ═══════════════════════════════════════════════════════════════════
...
fig4.suptitle(f"POS-Level Word Order by Structure (apr16, fixed budget)\n"
              f"(Dutch test, {n_seeds} seeds, tf=0 — blue = NL order, red = EN order)",
              fontsize=14, fontweight="bold")
...
print(f"\n=== KEY FINDINGS (NL test, {n_seeds} seeds) ===", flush=True)
