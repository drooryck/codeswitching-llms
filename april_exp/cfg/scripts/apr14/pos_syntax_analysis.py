...
follows the EN or NL template. This directly answers: "Which POS
position is responsible for language alignment?"
"""
...
def main():
    print("Starting divergent-position syntax analysis...", flush=True)

    # Pre-compute divergent positions
    div_info = {}
    ...
    # key = (prop, seed, test_lang, structure, div_pos_idx, en_role, nl_role)
    #   -> [n_en_match, n_nl_match, n_neither, n_total]
    agg = defaultdict(lambda: [0, 0, 0, 0])
    # Also aggregate by "role pair" across structures
    # key = (prop, seed, test_lang, en_role, nl_role) -> same
    ...
    # MAIN PLOT: NL test sentences, en_frac at divergent positions, by role pair
    ...
    fig.suptitle("Word-Order Alignment at Divergent Positions\n"
    ...
    # PER-STRUCTURE PLOT (NL test only)
    ...
    for idx, struct in enumerate(structs):
        ...
        if sub.empty:
            continue
        sub["role_pair"] = sub["en_role"] + "→" + sub["nl_role"]
        # Average over seeds, keep prop and role_pair
        avg = sub.groupby(["prop", "role_pair"])["en_frac"].mean().unstack()
        for rp in sorted(avg.columns):
            color = PAIR_COLORS.get(rp, "#333333")
            ax.plot(avg.index, avg[rp], "o-", label=rp, color=color,
    ...
    fig2.suptitle("Word-Order Alignment by Structure (NL test sentences)\n"
