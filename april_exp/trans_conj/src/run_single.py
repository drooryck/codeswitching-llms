"""
CLI entry point for running a single translation + conjugation experiment
...
def main():
    parser = argparse.ArgumentParser(
        description="Run a single trans_conj experiment (april_exp)")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--output-dir", type=str, required=True)
    parser.add_argument("--data-dir", type=str, required=True,
                        help="Dir containing train_pairs.csv / test_pairs.csv")
    parser.add_argument("--lexicon-path", type=str, required=True)
    parser.add_argument("--prop", type=float, required=True,
                        help="FR fraction in conjugation task (0-1). "
                             "Overall token fraction is reported exactly.")
