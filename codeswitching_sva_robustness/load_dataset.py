...
def get_valid_spanish_verbs_nouns(model):
    ...
    verb_list_tuples = list(set(examples_valid_verbs_tuples + read_files(model, "datasets/word_lists/plausible_spa_singular_plural_past_verbs.txt")))
...
def get_valid_french_verbs_nouns(model):
    ...
    verb_list_tuples = list(set(examples_valid_verbs_tuples + read_files(model, "datasets/word_lists/plausible_fr_singular_plural_past_verbs.txt")))  # Will add + read_files() later
    noun_list_tuples = list(set(examples_valid_nouns + read_files(model, "datasets/word_lists/fr_singular_plural_nouns.txt")))  # Will add + read_files() later
    return verb_list_tuples, noun_list_tuples, examples_valid_verbs_tuples_pred


def get_valid_dutch_verbs_nouns(model):
    """
    Load valid singular/plural noun and verb pairs for Dutch dataset generation.
    
    Args:
        model: The model used for tokenization.
    
    Returns:
        noun_list_tuples: List of (singular, plural) noun pairs.
        verb_list_tuples: List of (singular, plural) verb pairs.
        main_verb_tuples: List of (singular, plural) main verbs.
    """
    # Example manually curated valid words
    examples_valid_nouns = [
        ('minister', 'ministers'), 
        ('klant', 'klanten'), 
        ('student', 'studenten'), 
    ...
    # Verbs used in the **relative clause** ("die X deden")
    examples_valid_verbs_tuples = [
        ('aanviel', 'aanvielen'),  
        ('aankeek', 'aankeken'),
        ('aansprak', 'aanspraken'),
        ('hoorde', 'hoorden'),
    ...
    # Verbs used in the **main clause** ("werd/waren")
    examples_valid_verbs_tuples_pred = [
        ('werd', 'werden'),
        ('was', 'waren'),
        ('ging', 'gingen'),
        ('viel', 'vielen'),
    ...
    # Load additional words from dataset files
    noun_list_tuples = list(set(examples_valid_nouns))
    verb_list_tuples = list(set(examples_valid_verbs_tuples ))

    return noun_list_tuples, verb_list_tuples, examples_valid_verbs_tuples_pred


def load_sva_dataset(model, language, subject_number='both', split='train', num_samples=100, dataset_path=None):

    if dataset_path is None:
        dataset_path = os.path.join('datasets', 'final_datasets')
    filename = f"{lang}_{split}_sva_dataset.json"
...
def create_dataset(model, language):
    ...
    for split in ['train', 'validation', 'test']:
        final_datasets_dir = './datasets/final_datasets'
...
def create_dutch_dataset(model):
    """
    Create a Dutch Subject-Verb Agreement (SVA) dataset with singular/plural subject variations.

    Args:
        model: The model used for tokenization.

    Saves:
        JSON files for train, validation, and test sets in `./datasets/final_datasets/`.
    """
    noun_list_tuples, verb_list_tuples, main_verb_tuples = get_valid_dutch_verbs_nouns(model)
    
    noun_list_sing = [f"de {noun[0]}" for noun in noun_list_tuples]
    noun_list_plural = [f"de {noun[1]}" for noun in noun_list_tuples]

    verb_1_list_sing = [verb[0] for verb in verb_list_tuples]
    verb_1_list_plural = [verb[1] for verb in verb_list_tuples]

    verb_2_list_sing = [verb[0] for verb in main_verb_tuples]
    verb_2_list_plural = [verb[1] for verb in main_verb_tuples]

    # Generate all sentence permutations
    permutations_list = [
        [i, j, k] 
        for i in range(len(noun_list_sing)) 
        for j in range(len(verb_1_list_sing)) 
        for k in range(len(noun_list_sing)) 
        if k != i
    ]
    ...
    for valid_counter, (i, j, k) in enumerate(permutations_array):
        ...
        # Singular vs plural sentence construction
        sent_1 = f"{noun_list_sing[k]} die {noun_list_sing[i]} {verb_1_list_sing[j]} , "
        sent_2 = f"{noun_list_plural[k]} die {noun_list_sing[i]} {verb_1_list_plural[j]}, "
        ...
        # Randomly decide sentence order
        rdm_num = np.random.randint(2)
        ...
        # Assign subject number label
        base_subject_number = "plural" if base_label in verb_2_list_plural else "singular"

        final_dict[valid_counter]['src'] = src
        final_dict[valid_counter]['base'] = base
        final_dict[valid_counter]['src_label'] = src_label
        final_dict[valid_counter]['base_label'] = base_label
        final_dict[valid_counter]['base_subject_number'] = base_subject_number
    ...
    for split in ['train', 'validation', 'test']:
        ...
        output_path = f'{final_datasets_dir}/dutch_{split}_sva_dataset.json'
...
def create_sva_datasets(model):
    ...
    create_dataset(model, language="french")
    create_dataset(model, language="spanish")
    create_dutch_dataset(model)
